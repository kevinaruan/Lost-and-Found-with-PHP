...
<center>
    <h2 style="font-family:Bahnschrift Light Condensed;">
        <u>Daftar Barang Hilang Kategori NonElektronik</u>
    </h2>
</center>
<br>
...
<table class="table table-hover table-striped table-bordered" style="background-color:#c6d4e1">
    ...
    <tr>
        ...
        <center>
                ...
                <a href="http://localhost:8080/editproyek/mahasiswa/form/editHilang.php?id=<?= $baru['id'];?>"><input class="btn btn-primary" type="button" value="Edit"></a>
...
<?php
    if(!empty($_GET['id'])){
        $id = $_GET['id'];
        $status =  mysqli_query($data, "UPDATE formditemukan SET status = 1 WHERE id = $id");
    
        if(mysqli_affected_rows($data) > 0){
            header("Location: daftarbarangnon.php");
        }
}
?>
</center>

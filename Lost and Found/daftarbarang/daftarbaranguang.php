...
<center>
    <h2 style="font-family:Bahnschrift Light Condensed;">
        <u>Daftar Barang Ditemukan Kategori Uang</u>
    </h2>
</center>
<br>
...
<div>
    <form action="cariBarang.php" method="post">
        <input type="hidden" name="kategori" value="Uang">
...
<table class="table table-hover table-striped table-bordered" style="background-color:#c6d4e1">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <a href="daftarbaranguang.php?id=<?= $baru['id']; ?>"><button type="button" class="btn btn-secondary" data-toggle="modal" data-target="#myModal" name="ambil">Take</button></a>
        </div>
        
      </div>
    </div>
  </div>
                <a href="daftarbaranguang.php?id=<?= $baru['id']; ?>"><button class="btn btn-danger" name ="ambil">Delete</button></a>
              <?php }else{ ?>
                  <input type="button" class="btn btn-secondary" value="Take" disabled></a>
                  <a href="http://localhost:8080/editproyek/daftarbarang/deletebaranguang.php?id=<?= $baru['id']; ?>"><input type="button" class="btn btn-danger" value="Delete"></a>
...
<?php
    if(!empty($_GET['id'])){
        ...
        if(mysqli_affected_rows($data) > 0){
            echo "
                <script>
                    window.location.replace('daftarbaranguang.php');
                </script>
            ";
        }
}

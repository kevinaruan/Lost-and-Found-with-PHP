...
<center>
    <h2 style="font-family:Bahnschrift Light Condensed;">
        <u>Daftar Barang Ditemukan Kategori NonElektronik</u>
    </h2>
</center>
<br>
...
<div>
    <form action="cariBarang.php" method="post">
        <input type="hidden" name="kategori" value="NonElektronik">
...
<table class="table table-hover table-striped table-bordered" style="background-color:#c6d4e1">
    ...
    <?php $no = 0; foreach($hasil as $baru) :  
        if($baru['kategori'] == "NonElektronik") :
            $no++;
    ?>
    ...
    <tr>
        ...
        <?php
                if($baru['status'] == 0000-00-00){
                    ...
                    // $id = $_GET['id'];
                    // $del =  mysqli_query($data, "SELECT status FROM formditemukan WHERE id = $id");
                }
            ?>
        </td>
        <td></td>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <a href="daftarbarangnon.php?id=<?= $baru['id']; ?>"><button type="button" class="btn btn-secondary" data-toggle="modal" data-target="#myModal" name="ambil">Take</button></a>
        </div>
        
      </div>
    </div>
  </div>
                <a href="daftarbarangnon.php?id=<?= $baru['id']; ?>"><button class="btn btn-danger" name ="ambil">Delete</button></a>
              <?php }else{ ?>
                  <input type="button" class="btn btn-secondary" value="Take" disabled></a>
                  <a href="http://localhost:8080/editproyek/daftarbarang/deletebarangnon.php?id=<?= $baru['id']; ?>"><input type="button" class="btn btn-danger" value="Delete"></a>
...
<?php
    if(!empty($_GET['id'])){
        ...
        if(mysqli_affected_rows($data) > 0){
            echo "
                <script>
                    window.location.replace('daftarbarangnon.php');
                </script>
            ";
        }
}
